...
const SUBJECT_COLORS = {
  "Khmer": "#e84545", "Math": "#4169e1", "English": "#22a67a",
  "Science": "#9b59b6", "Social Studies": "#e67e22", "Art": "#e91e8c",
  "PE": "#27ae60", "Computer": "#2980b9", "Moral Education": "#795548",
  "Break": "#aaaaaa", "Khmer Review": "#c0392b", "Math Review": "#2c3e8c",
  "English Review": "#1a7a5a", "Science Review": "#7d3c98",
  "Exam Prep": "#c0392b", "Computer Lab": "#1a6090",
}
...
export default function WeeklySchedule({ weeklySchedule, color }) {
  const { lang } = useApp()
  const kh = lang === 'km'
    ...
    <div className="ws-wrap" style={{ "--cc": color }}>
      ...
      <div className="ws-day-tabs">
        {DAYS_EN.map((day, i) => (
          ...
          >
            <span className="ws-day-short">{kh ? DAYS_KH[i].slice(0, 2) : day.slice(0, 3)}</span>
            <span className="ws-day-full">{kh ? DAYS_KH[i] : day}</span>
      ...
      <div className="ws-day-heading">
        ...
        {kh ? DAYS_KH[DAYS_EN.indexOf(activeDay)] : activeDay}
        <span className="ws-slot-count">
          {slots.filter(s => s.subject !== "Break").length} {kh ? "មុខវិជ្ជា" : "classes"}
        </span>
      </div>
      ...
      <div className="ws-slots">
        {slots.map((slot, i) => {
          ...
          return (
            <div key={i} className={`ws-slot ${isBreak ? "ws-slot-break" : ""}`}
              ...
              {isBreak ? (
                ...
                <div className="ws-slot-body">
                  ...
                  <div className="ws-slot-name">{kh ? slot.subjectKh : slot.subject}</div>
                </div>
              )}
            </div>
          )
